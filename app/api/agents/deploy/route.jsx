...
export async function POST(req) {
  try {
    ...
    // Étape 1 : Créer le workflow (sans active)
    ...
    // Étape 2 : Activer le workflow séparément
    ...
    if (!activateRes.ok) {
      // Workflow créé mais pas activé — c'est quand même un succès partiel
      return NextResponse.json({
        success: true,
        activated: false,
        workflow_id: created.id,
        workflow_url: `${N8N_URL}/workflow/${created.id}`,
        message: 'Workflow créé ! Active-le manuellement dans n8n.'
      })
    }
    ...
    if (node.type === 'llm') {
      n8nNode = {
        ...
        parameters: {
          ...
          headerParameters: {
            parameters: [
              { name: 'x-api-key', value: process.env.ANTHROPIC_API_KEY || '' },
          ...
          bodyParameters: {
            parameters: [
              ...
              { name: 'messages', value: JSON.stringify([{ role: 'user', content: node.prompt_template }]) },
            ]
          },
        },
        position: [posX, 300],
